...
class ResourceManager extends EventDispatcher {
    ...
    get(resourceName) {
        ...
        console.error(`Resource manager "${this._name || this._namespace}" : Couln't find resource with name "${resourceName}"`);
    }
    ...
    add(resource) {
        ...
        if (this._isResourceAdded(resource)) {
            console.warn(`Resource manager "${this._name || this._namespace}" : Resource with same name or path has already been added for "${resource.name}"`);
            return;
        }
    ...
    addByName(resourceName) {
        ...
        for (let i = 0; i < ResourceLoader.resources.length; i++) {
            ...
            // Make sure the resource isn't already added
            if (this._isResourceAdded(resource)) {
                console.warn(`Resource manager "${this._name || this._namespace}" : Resource with name "${resource.name}" has already been added`);
        ...
        if (resourceToAdd === null) {
            console.warn(`Resource manager "${this._name || this._namespace}" : Could not find any resource with name "${resourceName}"`);
        }
    }
    ...
    load() {
        ...
        for (let i = 0; i < this._resources.length; i++) {
            ...
            const promise = ResourceLoader.load(resource.name);
    ...
    _addByNames(resourcesNames) {
        ...
        if (resourcesNames.length > 0) {
            console.warn(`Resource manager "${this._name || this._namespace}" : Could not find any resource with following name(s) : ${resourcesNames.join(', ')}`);
        }
    }
