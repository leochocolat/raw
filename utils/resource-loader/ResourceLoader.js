...
class ResourceLoader extends EventDispatcher {
    ...
    preload() {
        ...
        return Promise.all(promises).catch(this._preloadErrorHandler).then(this._preloadCompleteHandler);
    }
     ...
     * @returns {Promise}
     */
    static load(resourceName) {
        return this.loadResourceByName(resourceName);
    }
